...
# 检测已有文件
ls -la .aies/.ai/ .aies/ .claude/ .cursor/ 2>/dev/null
...
### Step 4：引导填写 .aies/.ai/index.md（关键）
...
用户回答后，Agent 生成并写入 `.aies/.ai/index.md`（按项目真实结构，不写 TODO）。
...
- `.aies/.ai/index.md` 必须填写真实内容，不能留 TODO
- 如果项目已有 `.aies/.ai/index.md`，读取并展示给用户，询问是否要更新
